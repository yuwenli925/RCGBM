...
totalFace = [elem(:,[2 3 4]); elem(:,[1 4 3]); ...
                    elem(:,[1 2 4]); elem(:,[1 3 2])];
totalFace = sort(totalFace,2);
[face, i2, j] = unique(totalFace,'rows','legacy');
i1(j(4*NT:-1:1)) = 4*NT:-1:1; i1 = i1';
k1 = ceil(i1/NT); t1 = i1 - NT*(k1-1);
k2 = ceil(i2/NT); t2 = i2 - NT*(k2-1);
idx = (i1 ~= i2); 
neighbor = accumarray([[t1(idx),k1(idx)];[t2,k2]],[t2(idx);t1],[NT 4]);
elem2face = reshape(j,NT,4);
face2elem = [t1,t2,k1,k2];
bdElem = t1(t1 == t2);
bdk1 = k1(t1 == t2);
...
%OriFace = cross(node(face(:,2)-face(:,1),:),node(face(:,3)-face(:,1),:));

T = struct('neighbor',neighbor,'elem2face',elem2face,'face',face,...
           'face2elem',face2elem,'bdElem',bdElem,'bdFace',bdFace,'bdFace2elem', bdFace2elem,...
           'bdFaceIndex',find(face2elem(:,1)==face2elem(:,2)));